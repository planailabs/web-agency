...
pub struct AsyncOperation {
    pub status: String,
    #[serde(rename = "type")] pub op_type: Option<String>,
...
pub struct ContactDetails {
    ...
    pub email: String, pub phone: String, pub address1: String,
    pub city: String, pub country: String,
    #[serde(skip_serializing_if = "Option::is_none")] pub organization: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] pub address2: Option<String>,
    #[serde(rename = "stateProvince", skip_serializing_if = "Option::is_none")] pub state_province: Option<String>,
    #[serde(rename = "postalCode", skip_serializing_if = "Option::is_none")] pub postal_code: Option<String>,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct ContactSaveResponse {
    #[serde(rename = "contactId")] pub contact_id: String,
}
...
pub struct DomainAvailability {
    ...
    #[serde(rename = "premiumPricing", default)] pub premium_pricing: Vec<PremiumPrice>,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct PremiumPrice {
    pub operation: String, pub price: f64, pub currency: String,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct DomainListResponse {
    pub items: Vec<DomainInfo>,
    #[serde(rename = "totalCount")] pub total_count: Option<u32>,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct DomainInfo {
    pub name: String,
    #[serde(rename = "autoRenew")] pub auto_renew: Option<bool>,
...
pub struct NameserverConfig {
    pub provider: String,
    #[serde(skip_serializing_if = "Option::is_none")] pub hosts: Option<Vec<String>>,
}
...
impl SimpleClient {
    ...
    async fn put_json<T: DeserializeOwned, B: serde::Serialize>(&self, path: &str, body: &B) -> Result<T, Error> {
    ...
    pub async fn poll_until_complete(&self, op_id: &str, timeout: Duration) -> Result<AsyncOperation, Error> {
        let deadline = tokio::time::Instant::now() + timeout;
        loop {
            let op = self.get_async_operation(op_id).await?;
            match op.status.as_str() {
                "success" => return Ok(op),
                "failed" => return Err(Error::AsyncFailed(serde_json::to_string(&op.details).unwrap_or_default())),
                _ => {
                    if tokio::time::Instant::now() > deadline { return Err(Error::AsyncTimeout); }
