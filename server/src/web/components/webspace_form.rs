...
async fn create_webspace(
    ...
    let id = sqlx::query_scalar::<_, Uuid>(
        "INSERT INTO webspaces (organization_id, name, hosting_type, runtime, relay_url, relay_credential_id, auth_mode, auth_basic_list_id) \
...
pub fn WebspaceForm() -> Element {
    ...
    let (org_list, mac_mgmt_creds, basic_auth_lists) = match &*data.read() {
        Some(Ok(d)) => (d.orgs.clone(), d.mac_mgmt_creds.clone(), d.basic_auth_lists.clone()),
        _ => (vec![], vec![], vec![]),
    };
            ...
            onsubmit: move |evt| {
                ...
                spawn(async move {
                    ...
                    if let Some(oid) = oid {
                        ...
                        match create_webspace(oid, n, ht, rt_opt, relay_url_opt, relay_cred_opt, am_val, ba_list_opt).await {
